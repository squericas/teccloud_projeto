...
function mostrarAdmin() {
    ...
    carregarCategorias(); // atualiza o select do artigo
...
if (authToggleBtn && authForm && authTitle && authSubmitBtn && emailGroup && authMessage && crudSection) {
    ...
    authForm.addEventListener('submit', async (e) => {
        ...
        const url = isLoginMode ? '/api/utilizador/login' : '/api/utilizador/register';
...
async function carregarCategoriasLista() {
    ...
    try {
        ...
        if (!categorias.length) {
            lista.innerHTML = '<p style="color:#c9c9d4;">Nenhuma categoria criada.</p>';
            return;
        }

        lista.innerHTML = categorias.map(c => `
            <div class="categoria-card" style="color:#FFF; margin="20px"">
                <strong>${c.id}</strong> -
...
if (categoriaForm) {
    categoriaForm.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (response.ok) {
                categoriaForm.reset();
                carregarCategoriasLista();   // Atualiza lista
                carregarCategorias();        // Atualiza o <select> do artigo
            } else {
    ...
    try {
        ...
        lista.innerHTML = artigos.map(a => `
            <div class="artigo-card" id="card-${a.id}">
                ...
                <span class="artigo-categoria">${a.Categoria?.nome || "Sem categoria"}</span>
                <p>${a.conteudo}</p>

                <div class="artigo-acoes">
                    <button class="btn-warning" onclick="mostrarFormularioEditar(${a.id}, '${encodeURIComponent(a.titulo)}', ${a.categoriaId}, '${encodeURIComponent(a.conteudo)}')">Editar</button>
                ...
                <div id="form-editar-${a.id}" style="display:none;" class="form-editar">
                    ...
                    <div class="form-group">
                        ...
                        <select id="edit-categoria-${a.id}" class="categoria-select"></select>
                    </div>
...
async function preencherSelectCategorias(selectElement, categoriaAtualId = null) {
    try {
        const response = await fetch('/api/categorias');
        const categorias = await response.json();

        selectElement.innerHTML = categorias
            .map(c => `
                <option value="${c.id}" ${c.id === categoriaAtualId ? "selected" : ""}>
                    ${c.nome}
                </option>
            `)
            .join('');
...
function mostrarFormularioEditar(id, titulo, categoriaId, conteudo) {
    ...
    preencherSelectCategorias(select, categoriaId);
}
...
async function guardarEdicao(id) {
    ...
    const categoriaId = document.getElementById(`edit-categoria-${id}`).value;
    ...
    try {
        const response = await fetch(`/api/artigos/${id}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ titulo, categoriaId, conteudo })
        });
...
if (artigoForm) {
    artigoForm.addEventListener('submit', async (e) => {
        ...
        const categoriaId = document.getElementById('artigo-categoria').value;
...
async function verificarSessao() {
    ...
    try {
        const response = await fetch('/api/utilizador/status');
        const data = await response.json();
